---
layout: page
title: Samples from Past Experiences
description: A collection of sample documentation supporting past tech docs, knowledgebase, and customer success experiences.
importance: 5
category: software-development
---
This section list a few samples from my prior tech writing experience. Note that some of these are proprietary information and may be water marked.

### Model N Systems — Principal Technical Writer

Focus: Procedural and reference documentation for Revenue Cloud for High Tech (RC HT), Life Science (Contracts & Pricing).

- [Creating Contracts]({{ "/assets/pdf/creating-contracts.pdf" | relative_url }})
- [Adjudicating Discrepancies]({{ "/assets/pdf/adjudicating-discrepancies.pdf" | relative_url }})
- [Chargebacks: Loading and Validating Lines]({{ "/assets/pdf/chargebacks-loading-and-validating-lines.pdf" | relative_url }})
- [Rebates Workflow]({{ "/assets/pdf/rebates-workflow.pdf" | relative_url }})
- [Assigning Accounts to a New Sales Org]({{ "/assets/pdf/assigning-accounts-to-new-sales-org.pdf" | relative_url }})
- [Release Notes: Winter 2017, Release Pack 1, Hotfix 6]({{ "/assets/pdf/release-notes-winter-2017-rp1-hf6.pdf" | relative_url }})


### Delphix Corp. — Staff Technical Writer

Focus: Knowledge Base strategy and process documentation.

- [Welcome to the Redesigned Knowledge Base]({{ "/assets/pdf/welcome-to-redesigned-knowledge-base.pdf" | relative_url }})
- [Working with Knowledge Management]({{ "/assets/pdf/knowledge-management-ijmt.pdf" | relative_url }})
- [Knowledge Content Style Guide]({{ "/assets/pdf/knowledge-content-style-guide.pdf" | relative_url }})


### Roku, Inc - Content Strategist

Focus: Customer Success & KNowledge Base strategy.

- Refer to content in [Roku Support](https://support.roku.com/en-us/)

<div class="row">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/roku-support-page.png" title="roku_support_page" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
